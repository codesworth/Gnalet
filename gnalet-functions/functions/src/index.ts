    ...
    try {
      if (oldcat !== afcat) {
        await admin.firestore().runTransaction(async transaction => {
          ...
          const beforenum =
            analyticdata.get(sup)[getStatusString(oldstatus)] - 1;
          const mbeforenum =
            analyticmonth.get(sup)[getStatusString(oldstatus)] - 1;
          const afternum =
            newanalyticdata.get(sup)[getStatusString(oldstatus)] + 1;
          const mafternum =
            newanalyticmonth.get(sup)[getStatusString(oldstatus)] + 1;
      ...
      if (oldstatus !== newstatus) {
        await admin.firestore().runTransaction(async transaction => {
          ...
          const beforenum =
            analyticdata.get(sup)[getStatusString(oldstatus)] - 1;
          const afternum =
            analyticdata.get(sup)[getStatusString(newstatus)] + 1;
          const mbeforenum =
            analyticmonth.get(sup)[getStatusString(oldstatus)] - 1;
          const mafternum =
            analyticmonth.get(sup)[getStatusString(newstatus)] + 1;
          const upd = analyticdata.get(sup);
          const mupd = analyticmonth.get(sup);
          upd[getStatusString(oldstatus)] = beforenum;
...
export const addNewAuthorityAccount = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      return Promise.resolve(
        "Succesfully Added Authority User Account: " + final
      );
    } catch (e) {
      console.log("Error Occurred with sig: ", e);
      return Promise.reject(e);
