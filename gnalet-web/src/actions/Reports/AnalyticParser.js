...
export class AnalyticParser {
  ...
  analyticsForPeriod = period => {
    ...
    switch (period) {
      ...
      case Duration.thisWeek:
        let start = moment().dayOfYear() - 6;
        if (start < 1) start = 1;
        this.range.start = start;
        this.range.end = moment().dayOfYear();
        return this.parseRange();

      case Duration.thisMonth:
        let mstart = moment().dayOfYear() - 29;
        if (mstart < 1) mstart = 1;
        this.range.start = mstart;
        this.range.end = moment().dayOfYear();
        return this.parseRange();
  ...
  parseRange = () => {
    ...
    console.log(`The range is: ${this.range.start} and ${this.range.end}`);
